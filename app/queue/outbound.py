...
class OutboundQueueEnvelope:
    ...
    @classmethod
    def from_dict(cls, raw: Mapping[str, Any]) -> OutboundQueueEnvelope:
        data = dict(raw or {})
        return cls(
            message_id=str(data.get("message_id") or ""),
            message_type=str(data.get("message_type") or ""),
            aggregate_type=str(data.get("aggregate_type") or ""),
...
@dataclass(frozen=True)
class OutboundQueueRecord(OutboundQueueEnvelope):
    ...
    @classmethod
    def from_dict(cls, raw: Mapping[str, Any]) -> OutboundQueueRecord:
        envelope = OutboundQueueEnvelope.from_dict(raw)
        data = dict(raw or {})
        return cls(
